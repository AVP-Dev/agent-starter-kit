...
Скрипт добавляет только отсутствующие файлы, не перезаписывает существующие `AGENTS.md`, `CLAUDE.md` и `docs/*.md`, а затем передаёт проект агенту для сканирования и заполнения. `--update` предназначен только для существующего проекта и не создаёт новый каталог.
